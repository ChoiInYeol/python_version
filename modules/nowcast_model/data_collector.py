...
def get_fred_series(series_id: str, api_key: str, freq: str) -> pd.DataFrame:
    ...
    try:
        ...
        if freq == 'd':
            ...
            resampled = series_data.set_index('date')['value'].resample('W').ffill()
        elif freq == 'm':
            resampled = series_data.set_index('date')['value'].resample('M').ffill()
        elif freq == 'q':
            resampled = series_data.set_index('date')['value'].resample('Q').ffill()
        else:
...
def collect_release_data(variable_params: pd.DataFrame, releases_info: pd.DataFrame) -> Dict:
    """
    릴리스 데이터를 수집합니다.
    
    Args:
        variable_params (pd.DataFrame): 변수 정의 데이터프레임
        releases_info (pd.DataFrame): 릴리스 정보 데이터프레임
    ...
    try:
        ...
        # FRED 릴리스 필터링
        fred_releases = releases_info[releases_info['source'] == 'fred']
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_release):
                ...
                try:
                    ...
                    release_dates['release'] = release_id
