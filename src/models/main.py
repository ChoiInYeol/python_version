...
def main():
    ...
    model_type = 'midas'  # 원하는 모델로 변경 가능
    ...
    if model_type == 'midas':
        model = MIDASModel(
            ...
            target='CPI_YOY',
            lookback_periods=12,  # 월별 기간
            poly_degree=2,        # 가중치 함수의 다항식 차수
            max_lags={'default': 1},  # 모든 변수에 대한 기본 30일 지연
            train_window_size=730,  # 일
            output_dir=f'output/nowcasts_{model_type}_{data_name}'
        )
    else:
        model = DFMModel(
            X_path=f'data/processed/X_{data_name}.csv',
    ...
    # 전체 Nowcast 출력
    if model_type == 'midas':
        model.plot_results()  # MIDAS 모델은 plot_results() 내에서 CSV 저장도 수행
    else:
    ...
    if model_type == 'midas':
        nowcast_df = pd.read_csv(f'output/nowcasts_{model_type}_{data_name}/nowcast_results.csv', index_col=0, parse_dates=True)
        ...
        if today in nowcast_df.index:
            ...
            print(f"  - Predicted CPI YoY (MIDAS): {today_pred:.2f}%")
